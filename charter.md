...
### 3.5 완료의 정의 — 대시보드에 보이지 않으면 완료가 아니다 *(신설 v0.5 — 2026-08-30)*

> **어떤 산출물도 대시보드에서 보이기 전까지는 완료가 아니다. 저장소에만 있는 것은 아무도 모르는 것과 같다.**

이 문장은 권고가 아니라 §3.4 척추와 같은 급의 **불변식**이다. 어기면 그 작업은 완료로 셀 수 없다.

#### 왜 이것이 규칙이어야 하나

Atelier의 의사결정자는 저장소를 열어 파일 트리를 뒤지지 않는다. 대시보드 하나만 본다. 그것이 이 시스템의 설계 전제다(§8 사용자와 Agent의 관계). 그런데 그 전제가 지켜지는지를 아무도 재지 않았고, 그 결과가 실측으로 드러났다.

**2026-08-30 실측.** 한 세션이 mobile-rosary 안건에서 다음을 만들어 저장소에 푸시했다 — 경쟁사 실화면 36장을 분석한 디자인 조사, 전면 재작성한 PRD, 새 경쟁자(Rosario) 분석, 프로토타입 v4, 다른 AI에 보낼 시안 브리프 넷, 그리고 기도 단계 수의 오류 정정. **그중 대시보드에 올라간 것은 0건이었다.** 대시보드는 그보다 이틀 전 상태에 멈춰 있었고, PRD 파일 자체가 참조 목록에 없었다.

작업이 부실했기 때문이 아니다. **완료의 정의가 "저장소에 푸시했다"에서 멈춰 있었기 때문이다.** 이 절은 그 정의를 끝까지 밀어 놓는다.

#### 무엇이 완료인가 — 세 가지를 모두 만족해야 한다

| 조건 | 뜻 | 어기면 |
|---|---|---|
| **1. 만들었다** | 산출물이 저장소에 있다 | 아무것도 없다 |
| **2. 닿는다** | 대시보드에서 클릭 한 번으로 그 산출물에 도달한다 | **"만들었으나 아무도 모른다" — 완료 아님** |
| **3. 읽힌다** | 대시보드에 그것이 무엇이고 무슨 상태인지가 적혀 있다 | 링크는 있으나 왜 봐야 하는지 모른다 |

**2번이 이 규칙의 핵심이다.** 파일 경로를 채팅에 적는 것은 닿는 것이 아니다. 의사결정자는 그 경로를 기억하지 않는다. 대시보드에서 출발해 클릭으로 도달할 수 있어야 한다.

#### 이 규칙이 기존 규칙과 어떻게 맞물리나

공용 커뮤니케이션 규칙 7-2의 **노출 증명**(사용자가 정상 진입점에서 산출물에 닿을 수 있다는 증명)이 이 규칙의 일반형이다. 이 절은 Atelier에서 **그 "정상 진입점"이 무엇인지를 확정한다 — 대시보드다.** 규칙 7의 세 단계 어휘도 여기에 맞춰 읽는다.

- **작업완료**: 만들고 검증해 작업 브랜치에 푸시했다. **여기까지는 아직 완료가 아니다.**
- **검토대기**: PR이 열려 사람의 판단을 기다린다.
- **배포완료**: main에 머지됐고, **대시보드에서 보인다.** 꾸밈없는 "완료"는 이 상태에만 쓴다.

#### 지킬 방법 — 산출물과 대시보드를 같은 PR에 넣는다

문서를 새로 만들거나 크게 고쳤으면 **그 PR 안에서 대시보드도 함께 갱신한다.** 나중에 하겠다는 것은 하지 않겠다는 것과 같다. 실제로 위 사고가 그렇게 일어났다.

보고할 때도 같다. 산출물을 보고하면서 **"대시보드 어디에서 보이는가"를 한 줄로 적는다.** 그 줄을 쓸 수 없으면 그 작업은 완료가 아니며, 보고에 "대시보드 미반영"을 명시한다. 금지되는 것은 미반영 자체가 아니라 **미반영에 대한 침묵**이다.

#### 이 규칙이 드러내는 구조적 약점

대시보드가 **사람이 손으로 갱신하는 문서**인 한, 이 규칙은 매번 사람의 기억에 기댄다. 기억에 기대는 장치는 반드시 새어 나간다 — 위 사고가 그 증거다. 그러므로 **대시보드는 저장소의 정본 자료를 읽어 스스로 만들어지는 것으로 바꿔야 한다.** 그때 비로소 이 규칙이 규율이 아니라 구조가 된다.
...
## 5. 핵심 프로세스 — 9단계 × 3영역, 5 묶음 *(개정 v0.6 — 2026-09-05, §5.1-1 신설)*
...
#### 5.1-1 격자를 화면에 그린다 — 빈 칸의 권리는 빈 칸을 보여야 성립한다 *(신설 v0.6 — 2026-09-05)*

위 문단이 "비어 있을 권리를 보장한다"고 적었는데, **그 권리가 두 해 가까이 집행되지 않았다.** 2026-09-05 에 현황판을 실측하니 27칸을 실제로 그린 곳은 01·02 카드뿐이었고, 03~05 는 본문 소제목으로만 남았으며 **06 이후에는 격자가 통째로 사라져 있었다.** 카드는 대신 회차 기록이 위로 쌓이는 게시판이 되어 있었다.

칸이 화면에서 사라지면 두 가지가 같아 보인다 — **의도적으로 비운 칸**과 **빠뜨린 칸**이다. 실제로 그 구분이 무너져 두 곳이 조용히 비어 있었다. ① 05단계가 접는 기준을 두 줄(완주 50% · 구매 20%)로 못 박고 결정 큐가 그것을 닫았는데, **정작 그 숫자를 재는 08단계 카드에 두 지표가 한 번도 적히지 않았다.** ② 2026-09-05 정정으로 계정과 서버가 생겨 05 의 비용 산정이 무효가 됐는데, **06 의 사업 칸이 비어 있어 그 사실이 어디에도 보이지 않았다.** 둘 다 규칙 위반이 아니라 배선 누락이며, §3.5(대시보드에 보이지 않으면 완료가 아니다)의 격자판이다.

그래서 앞으로 단계 카드를 만들고 갱신하는 규약을 넷으로 못 박는다.

| # | 규약 | 왜 |
|---|---|---|
| ① | **카드는 3영역 칸을 맨 위에 먼저 보인다** | 카드를 열었을 때 첫 화면이 회차 기록이면 "지금 무엇이 있나"에 답하지 못한다 |
| ② | **빈 칸을 지우지 않고 빈 칸으로 그린다** | 칸이 사라지면 의도와 누락이 같아 보인다 |
| ③ | **빈 칸마다 둘 중 하나를 적는다** — "해당 없음(왜 없는지)" 또는 "채워야 함(누가 언제)" | 이유 없는 빈 칸은 다음 사람에게 아무것도 알려 주지 않는다 |
| ④ | **경위는 카드에 쌓지 않는다.** 회차 기록은 `decisions.md` 가 갖고 카드는 결과만 말한다 | 카드가 게시판이 되면 결과가 기록 아래로 밀린다 |

**격자 전체의 상태는 현황판의 `격자` 카드 한 장이 보인다.** 어느 칸이 찼고, 절반이고, 비었고, 해당 없는지를 한 화면에서 읽을 수 있어야 이 규약이 실제로 작동한다.
...
### 3.6 판단의 표면은 화면 하나다 *(신설 v0.5)*

§3.5가 "대시보드에서 보여야 완료"라고 정했다면, 이 절은 **그 대시보드가 어느 것인지**를 못 박는다. 둘 이상이면 §3.5는 지켜도 소용이 없다 — 보는 사람이 어느 쪽을 봐야 할지 모르기 때문이다.

> **정본 현황판은 `workflows/type1-external-service.html`(기획 현황판) 하나다. 다른 현황판을 새로 만들지 않는다.**

이것은 새 결정이 아니라 [`decisions.md`](./decisions.md) **결정 1(2026-08-27)** 의 재확인이다. 그때 8월에 만든 progress-dashboard 체계를 `archive/`로 옮기며 이렇게 적었다 — *"두 체계는 전제가 달랐다. 8월 것은 2단계 구조와 작업 패키지 어휘 위에, 정본은 9단계 × 3영역 위에 세워졌으므로 합칠 수 없었다. 판단의 표면은 이제 화면 하나다."*

**그럼에도 2026-09-01에 한 세션이 그 폐기된 체계를 되살려 두 번째 현황판을 만들었다.** 헌장 §3.5를 지키려다 정작 §3.5가 가리키는 대상을 둘로 늘린 것이다. 되돌렸고, 그 사고를 막기 위해 이 절을 남긴다.

**그러므로 대시보드를 고치는 일은 언제나 정본 하나를 고치는 일이다.** 새 현황판을 만들고 싶어지면 그것은 대개 정본이 낡았다는 신호이지 새 판이 필요하다는 신호가 아니다. 정본이 갱신하기 어려워서 새로 만들고 싶은 것이라면, **새로 만들지 말고 정본을 갱신하기 쉽게 고친다.**

그 조치는 2026-09-01에 실행됐다. 정본을 껍데기와 카드 조각으로 갈라 [`workflows/board/`](./workflows/board/README.md)에 두었고, 이제 세션은 300KB 덩어리가 아니라 **카드 한 장(수 KB)을 고치고 조립기를 돌린다.** 목차와 카드가 어긋나면 조립이 멈추므로, 상태 표기가 조용히 낡을 수 없다.

**그리고 이 절이 글로만 남지 않도록 강제 장치를 걸었다.** 폐기된 progress-dashboard 체계는 이 저장소의 폐기 목록 [`.claude/retired.json`](./.claude/retired.json)에 올라가 있고, 훅(`.claude/hooks/retired-guard.py`)이 그 스킬의 호출과 폐기된 자리(`meta/` · `ai-npi/` · `dashboard.config.json` · `docs/dashboard.html` · `STATUS.md`)로의 쓰기를 실제로 차단한다. 세션이 시작될 때 그 목록이 자동으로 실리므로, 이 절을 읽지 않은 세션도 같은 사고를 반복할 수 없다. 목록의 형식과 되살리는 절차는 공용 규칙 `.claude/rules/past-decisions.md` 가 정한다 — **폐기를 되돌리려면 사람의 결정을 받아 목록에서 항목을 지워야 하며, 그 삭제는 커밋에 남는다.**
