...
**그럼에도 2026-08-30에 한 세션이 그 폐기된 체계를 되살려 두 번째 현황판을 만들었다.** 헌장 §3.5를 지키려다 정작 §3.5가 가리키는 대상을 둘로 늘린 것이다. 되돌렸고, 그 사고를 막기 위해 이 절을 남긴다.
...
그 조치는 2026-08-30에 실행됐다. 정본을 껍데기와 카드 조각으로 갈라 [`workflows/board/`](./workflows/board/README.md)에 두었고, 이제 세션은 300KB 덩어리가 아니라 **카드 한 장(수 KB)을 고치고 조립기를 돌린다.** 목차와 카드가 어긋나면 조립이 멈추므로, 상태 표기가 조용히 낡을 수 없다.
