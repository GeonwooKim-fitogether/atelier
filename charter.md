...
- **v0.1** (2026-05-XX): 초안 박제
- **v0.1.1** (2026-05-XX): 시드 시스템 §9.3 비대칭 명시 + Foundry 미해결 추가
- **v0.2** (2026-05-21):
  - §1.2 모순 정정 (MVP 제작 → 프로토타입 제작)
  - §1.3 Handshake = PRD 패키지로 정의
  - §3.3 AI 시대 속도 원칙 신설
  - §4.3 24h 묵힘 두 항목 모두 제거
  - §5 전면 재작성 (Stage 1/2 작성 권한 대칭 + 용어 정의)
  - §6 흐름 재작성 (Flywheel outline)
  - §9.1~9.5 시간 강제 제거 + 순서 정정
  - §9.6 idea-funnel 검토 마일스톤 trigger
  - §13 거부 목록 갱신 (도구 list 박지 않음 + 시간 강제 + FM 흡수 추가)
  - §16 미해결 갱신
  - **Workflow 외부화** → `.claude/skills/atelier-npi/SKILL.md`
