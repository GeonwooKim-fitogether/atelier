...
### 3.6 상태 어휘 — STATUS.md가 쓰는 다섯 표시 *(신설 v0.5)*

§3.5의 완료 정의를 장부에 옮긴 표기다. 저장소 루트의 `STATUS.md`가 이 표시를 쓰고, 대시보드가 그것을 읽어 열을 정한다.

| 표시 | 뜻 | 대시보드에서 |
|---|---|---|
| 체크 표시 | **배포완료** — main에 있고 대시보드에서 보인다 | Done |
| 모래시계 | **검토대기** — 작업 브랜치에 있고 머지를 기다린다. **아직 완료가 아니다** | Verification |
| 노란 원 | 부분 구현 — 만들다 만 것 | Development |
| 가위표 | 미착수 | Backlog |
| 일시정지 | 의도적 보류 — 지금 안 하기로 정한 것 | Backlog |
| 갈림길 | 열린 결정 — 사람이 답해야 진행된다 | 결정 큐 |

산출물을 만든 세션은 **그 PR 안에서 `STATUS.md`의 해당 줄을 함께 고친다.** 이것이 §3.5를 지키는 유일한 조작이며, 그 밖에 대시보드를 손으로 만질 일은 없다.
